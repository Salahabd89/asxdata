...
async function getAllCompanies() {
    ...
    await Promise.all(letters.map(async function (value, i) {

         var page = await getASXCompanies(letters[i], browser1)
         all_c.push(page)
    ...
    for (let i = 0; i < all_c.length; i++) {
        for (let j = 0; j < all_c[i].length; j++)
            try {
                ...
                if (asx200list.indexOf(ASX_Code) > -1) {
    ...
    fs.writeFile("C:/Users/m047207/Downloads/download.csv", csv, function(err) {
        if(err) {
            console.log(err);
            return;
        }
